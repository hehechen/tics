...
inline TiDBTimestamp getTs(const TiKVKey & key)
{
...
{
    return TiKVKey(String(key.data(), key.dataSize() - sizeof(TiDBTimestamp)));
}

inline TiKVKey appendTs(const TiKVKey & key, TiDBTimestamp ts)
{
...
inline TiKVKey genKey(TableID tableId, HandleID handleId, TiDBTimestamp ts)
{
...
inline DecodedWriteCFValue decodeWriteCfValue(const TiKVValue & value)
{
    ...
    TiDBTimestamp prewrite_ts = RecordKVFormat::readVarUInt(data, len); // ts
...
inline TiKVValue encodeWriteCfValue(UInt8 write_type, TiDBTimestamp ts, std::string_view short_value = {}, bool gc_fence = false)
{
    WriteBufferFromOwnString res;
